...
export default function Tareas() {
  ...
  const editarTarea = async (tarea) => {
    const { value: formValues } = await Swal.fire({
      title: "Editar Tarea",
      html: `
      <input id="swal-nombre" class="swal2-input" placeholder="Nombre" value="${tarea.nombre_tarea}" />
      
      <input id="swal-fecha" type="date" class="swal2-input" value="${tarea.fecha_creacion.split("T")[0]}" />
    ...
    try {
      const res = await api.put(`/actualizarTarea/${tarea.id}`, {
        nombre_tarea: formValues.nombre,
        fecha_creacion: new Date(formValues.fecha).toISOString(),
        estado: formValues.estado
      });
      ...
      setTareas(prev =>
        prev.map(t =>
          t.id === tarea.id ? res.data.tarea : t
        )
      );
          ...
          {filtrarTareas.map((tarea) => (
            <div key={tarea.id} className="tarea-card">
              <div className="card-header">
                ...
                <EditIcon onClick={() => editarTarea(tarea)} />
              </div>

...
exports.actualizarTarea = async (req, res) => {
    const { id_usuario } = req.params;
    const { nombre_tarea, estado, fecha_creacion } = req.body;

    try {
        const result = await db.query(
            "UPDATE tareas SET nombre_tarea = $1, estado = $2, fecha_creacion = $3 WHERE id = $4 RETURNING *",
            [nombre_tarea, estado, fecha_creacion, id_usuario]
        );
